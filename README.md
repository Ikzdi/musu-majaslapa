...
- **Izaugsmes atskaņojums** (`.gr`, `index.html`) — velkams SVG grafiks: reāla klienta
  ...
  līkni, +140% nozīmīte. Velkams (bez rAF) → strādā arī ar reduced-motion.
- **Live Lighthouse** (`.lh`, `index.html`) — četri molten gauge-gredzeni (100/100/99/100)
  aizpildās ieritinot (`stroke-dashoffset`) + audita log. Reduced-motion → aizpildīti uzreiz.
- **Kūstošie virsraksti** (`.section__title.is-forged`, `main.js`) — sekciju virsraksti
  “atdziest” no molten-oranžas kvēles uz normālu (CSS `forgeCool`). Tikai motion;
  robusts pret daudzrindu/`<em>` virsrakstiem (nevis WebGL teksta-tekstūra).
